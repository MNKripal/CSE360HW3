	...
	public static String evaluatePassword(String input) {
		...
		while (running) {
			...
			if (currentChar >= 'A' && currentChar <= 'Z') {
				...
				return "An invalid character has been found in your password , only the following characters are allowed: A-Z, a-z, 0-9, ~`!@#$%^&*()_+{}[]:,.?/";
			}
			if (currentCharNdx >= 7) {
		...
		if (!foundLongEnough)
			errMessage += "Long Enough (<8 letters); ";
		
		if (errMessage == "")
			return "";
		
		passwordIndexofError = currentCharNdx;
		return "The following requirements weren't met for your password: " + errMessage;
